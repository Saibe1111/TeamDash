...
/**
* author: TeamDash
* description: controller for the user
...
class User extends \Controller {
    ...
    */
    private function pwdIsValid(string $pwd) {
        ...
        if (strlen($pwd) < User::MIN_LENGHT) {
            return !$valid;
        }

        if(preg_match(User::UPPERCASE, $pwd) == 0) {
            return !$valid;
        }

        if (preg_match(User::LOWERCASE, $pwd) == 0) {
            return !$valid;
        }

        if (preg_match(User::SPECIAL, $pwd) == 0) {
            return !$valid;
        }
